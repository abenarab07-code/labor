...
import { ArrowUpRight, FlaskConical, Microscope, Stethoscope } from "lucide-react";
...
export function LaboratoryAnalysisStage() {
  ...
  const smooth = useSpring(scrollYProgress, { stiffness: 100, damping: 24, mass: 0.35 });
  const stageScale = useTransform(smooth, [0, 0.08, 0.92, 1], [0.97, 1, 1, 0.96]);
  const stageOpacity = useTransform(smooth, [0, 0.05, 0.95, 1], [0.55, 1, 1, 0.55]);
  ...
  return (
    <section
      id="analyses"
      ref={sectionRef}
      className="relative scroll-mt-24 bg-plasma md:h-[300vh]"
    >
          ...
          <div className="col-span-6 max-w-[620px] pr-10">
            ...
            <AnimatePresence mode="sync">
              ...
              >
                ...
                <p className="mt-7 max-w-lg text-lg leading-8 text-slate">{current.text}</p>
